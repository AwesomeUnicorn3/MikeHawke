...
onready var camera = "res://Scenes/MikeHawke/Camera2D"
				...
				if next != null:
					...
					match this.node_type:
						...
						"show_message":
							...
							if this.slide_camera:
								pass
#								slide_camera_to(current_speaker)
...
#func slide_camera_to(object):
#	if is_box:
#		camera.set_global_position(object.global_position + Vector2(0, 64))
#	else:
#		camera.set_global_position(object.global_position + Vector2(0, -16))
#
#func return_camera():
#	camera.set_global_position(G.player().get_global_position())
